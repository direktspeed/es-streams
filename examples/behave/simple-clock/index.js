...
const click10ms = constant(periodic(10), click(qs('[name="10ms"]')))
const click100ms = constant(periodic(100), click(qs('[name="100ms"]')))
const click1s = constant(periodic(1000), click(qs('[name="1s"]')))

const clicks = mergeArray([click10ms, click100ms, click1s])
...
// have been clicked.
const sampler = switchLatest(startWith(periodic(1000), clicks))

// Get the elapsed time by sampling time() at the associated rate each
// time a button is clicked.
const elapsed = sample(time, sampler)
