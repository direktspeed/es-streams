...
class At {
    ...
    run(sink, scheduler) {
        return scheduleDelay(this.time, propagateTask(runAt, this.value, sink), scheduler);
    }
}
...
// tryDisposeErrorToStreamSignalEndToStream
/*
    tryDisposeWithErrorEnd
*/
export function tryDispose(error, end, x, disposable) {
    try {
        disposable.dispose();
    }
    catch (e) {
...
class Scan {
    ...
    run(sink, scheduler) {
        const d1 = scheduleAsap(propagateEventTask(this.value, sink), scheduler);
...
class ContinueWithSink extends Pipe {
    ...
    event(t, x) {
        ...
        tryDisposeErrorToSink(t, this.disposable, this.sink);
...
class CombineSink extends Pipe {
    ...
    dispose(t, index) {
        tryDisposeErrorToSink(t, this.disposables[index], this.sink);
...
class Outer {
    ...
    endInner(t, inner) {
        ...
        tryDisposeErrorToSink(t, inner, this);
...
const emitZipped = (f, t, buffers, sink) => sink.event(t, invoke(f, mapArray(head, buffers)));
...
class DebounceSink {
    ...
    event(_t, x) {
        this.clearTimer();
        this.value = x;
        this.timer = scheduleDelay(this.dt, new DebounceTask(this, x), this.scheduler);
    }
...
class ErrorStream {
    ...
    run(sink, scheduler) {
        return scheduleAsap(propagateErrorTask(this.value, sink), scheduler);
    }
}
class RecoverWith {
    constructor(f, source) {
...
// fromDomEvent :: (EventTarget t<string>, Event e) => String -> t -> boolean=false -> Stream e
export const fromDomEvent = (event, node, capture = false) =>
    new FromDomEvent(event, node, capture)

const runEffects = (stream, scheduler) => new Promise((resolve, reject) => runStream(stream, scheduler, resolve, reject))
export { MulticastSource, ap, at, awaitPromises, chain, combine, combineArray, concatMap, constant, continueWith, debounce, delay, during, empty, filter, fromPromise, join, loop, map, merge, mergeArray, mergeConcurrently, mergeMapConcurrently, multicast, never, newStream, now, periodic, propagateEndTask, propagateErrorTask, propagateEventTask, propagateTask, recoverWith, run, runEffects, sample, scan, since, skip, skipAfter, skipRepeats, skipRepeatsWith, skipWhile, slice, snapshot, startWith, switchLatest, take, takeWhile, tap, throttle, throwError, until, withItems, withLocalTime, zip, zipArray, zipItems };
