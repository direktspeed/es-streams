...
const deferTask = (task) => Promise.resolve(task).then(runTask);
 ...
 */
export const scheduleAsap = curry2((task, scheduler) => scheduler.scheduleTask(0, 0, -1, task));
/**
...
const scheduleDelay = curry3((delay, task, scheduler) => scheduler.scheduleTask(0, delay, -1, task));
/**
 * Schedule a task to run periodically, with the
 * first run starting asap
 */
const schedulePeriodic = curry3((period, task, scheduler) => scheduler.scheduleTask(0, 0, period, task));
/**
...
export { HRTimeClock, RelativeClock, cancelAllTasks, cancelTask, clockRelativeTo, currentTime, scheduleDelay, newClockTimer, newDateClock, newDefaultScheduler, newDefaultTimer, newHRTimeClock, newPerformanceClock, newPlatformClock, newScheduler, newTimeline, schedulePeriodic, schedulerRelativeTo };
